/******************************************************************************
    Stealth Injection Shellcode for Thread Hijacking - V3
    ...
    FIXES in V3:
    - Proper MASM FRAME directives for stack unwinding
    - Shadow space included in frame (no per-call sub rsp)
    - Exception directory registration (SEH support)
    - CFG-aware structure (LoadConfig directory)
 ******************************************************************************/
#pragma once
...
typedef struct _HIJACK_CONTEXT
{
    ...
    volatile LONG ErrorCode;         // 0x0C: Error code if failed (1=general, 2=import)
    ...
    // Must be 16-byte aligned for movaps
    ...
    ULONG64 EntryPointRva;           // 0x110: AddressOfEntryPoint RVA (0 if no EP)
    ...
    ULONG64 OriginalImageBase;       // 0x138: Original PE ImageBase for delta
    ULONG64 TlsDirRva;               // 0x140: TLS directory RVA
    ULONG64 TlsDirSize;              // 0x148: TLS directory size
    ULONG64 ExceptionDirRva;         // 0x150: Exception directory RVA (for SEH)
    ULONG64 ExceptionDirSize;        // 0x158: Exception directory size
    ULONG64 LoadConfigDirRva;        // 0x160: Load Config directory RVA (for CFG)
    ULONG64 LoadConfigDirSize;       // 0x168: Load Config directory size

    // === Required function pointers (offset 0x170) ===
    ULONG64 pLoadLibraryA;           // 0x170: kernel32!LoadLibraryA
    ULONG64 pGetProcAddress;         // 0x178: kernel32!GetProcAddress
    ULONG64 pNtFlushInstructionCache;// 0x180: ntdll!NtFlushInstructionCache
    ULONG64 pLdrpHandleTlsData;      // 0x188: ntdll!LdrpHandleTlsData (optional)
    ULONG64 pRtlAddVectoredExceptionHandler; // 0x190: ntdll (optional)
    ULONG64 pRtlAddFunctionTable;    // 0x198: ntdll (optional, for SEH)
    ULONG64 pRtlInsertInvertedFunctionTable; // 0x1A0: ntdll (optional, for CFG)

} HIJACK_CONTEXT, *PHIJACK_CONTEXT;
#pragma pack(pop)

static_assert(sizeof(HIJACK_CONTEXT) == 0x1A8, "HIJACK_CONTEXT size mismatch with ASM");

// Verify critical offsets match ASM
static_assert(offsetof(HIJACK_CONTEXT, ImageBase) == 0x100, "ImageBase offset mismatch");
static_assert(offsetof(HIJACK_CONTEXT, pLoadLibraryA) == 0x170, "pLoadLibraryA offset mismatch");
static_assert(offsetof(HIJACK_CONTEXT, OriginalXmm0) == 0xA0, "XMM0 offset mismatch");

//------------------------------------------------------------------------------
// Extern declarations for the assembled shellcode
//------------------------------------------------------------------------------
extern "C" void HijackShellcodeV2(PHIJACK_CONTEXT Context);
extern "C" UCHAR HijackShellcodeV2End;  // Label marking end of shellcode

// Calculate shellcode size from linker symbols
